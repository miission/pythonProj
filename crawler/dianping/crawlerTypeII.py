...
for firstClassIdx in [5,6]:
    ...
    for foodType in foodTypes[1:]:
        ...
        while(subFoodTypeUrl!='None'):
            ...
            for foodContent in foodContents:  
                ...
                '''get titles'''
                try:
                   brand = foddDetail.find('p',{'class':'title'}).findAll('a')[0].text.replace('\n','') 
                except:
                    next
                try:
                    promoSlogns = foddDetail.find('div',{'class':'tuan-info'}).findAll('a')
                ...
                try:
                    meanPrice  =p.findall(foddDetail.find('p',{'class':'average'}).text)[0]
                except:
                    meanPrice = -1
                '''get address'''
                try:
                    address = foddDetail.find('p',{'class':'area-list'}).text.replace('\n','').replace(' ','')
                except:
                    try:
                        address = foddDetail.find('p',{'class':'key-list'}).text.replace('\n','').replace(' ','')
                    except:
                        address = -1
                allFoodList.append([subFoodTypeName,brand,promoSlogn,meanPrice,address]+rate)
    ...
    BrandDF = pd.DataFrame(allFoodList,columns=[u'所属分类',u'店面招牌',u'优惠措施',u'平均消费',u'店面地址',u'评价等级',u'评价数量',u'产品数量'])
    ...
    BrandDF.to_csv(dirs,encoding='gb18030',index=False,columns=[u'所属分类',u'店面招牌',u'优惠措施',u'平均消费',u'店面地址',u'评价等级',u'评价数量',u'产品数量'])
